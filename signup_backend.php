...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	if($result->num_rows>0){
		echo "<script>alert('User with email-id exists.');window.location='login2.php';</script>";
		die();
	}
	else
	{
		...
		$user_id = $arr['id'];
		$_SESSION['id'] = $user_id;
		echo "<script>alert('Successfully Signed Up');window.location='Homepage.php';</script>";
		die();
	}
